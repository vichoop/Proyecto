...
#define SueloPin_T2 A0 // Definición del Pin Análogo del sensor de Humedad del Suelo.

DHT dht_T2(DHTPIN, DHTTYPE); // Ejecución de la librería del DHT y nombre del sensor: 'dht_T2'.
LiquidCrystal lcd(8, 9, 4, 5, 6, 7); // Ejecución de la libraría del LCD y Configuración Pines Pantalla LCD (rs, en, d4, d5, d6, d7).
Adafruit_CCS811 ccs_T2; // Ejecución de la librería del CCS811 y nombre del sensor 'ccs_T2'.
...
void loop() {
  ...
  float Temperatura = dht_T2.readTemperature(); // Clasificación de Variable de Temperatura (en C°).
  float HA = dht_T2.readHumidity(); // Clasificación de Variable de Humedad Ambiental (en %).
  int  HS=map(analogRead(SueloPin_T2), 0, 1023, 100, 0); // Clasificación de Variable de Humedad del Suelo (en %).
  float co2 = ccs_T2.geteCO2(); // Clasificación de Variable de Dióxido de Carbono (en ppm).
